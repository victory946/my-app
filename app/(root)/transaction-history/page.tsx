...
const TransactionHistory = async ({ searchParams }: SearchParamProps) => {
  ...
  if (!loggedIn) return <p className="text-red-500">Error: Please log in to view transactions.</p>;

  // ✅ Fetch user accounts
  const accounts = await getAccounts({ userId: loggedIn.$id });

  if (!accounts?.data || accounts.data.length === 0) {
    return <p className="text-red-500">Error: No linked bank accounts found.</p>;
  }
  ...
  if (!account || !account.transactions) {
    return <p className="text-red-500">Error: Account data not available.</p>;
  }
